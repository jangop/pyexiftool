...
class ExifToolHelper(ExifTool):
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    def __init__(
        self, executable=None, common_args=None, win_shell=True, return_tuple=False
    ):
    ...
    def execute_json_wrapper(self, filenames, params=None, retry_on_error=True):
        ...
        if result:
            try:
                ...
                if retry_on_error:
                    result = self.execute_json_filenames(
                        filenames, params, retry_on_error=False
                    )
                else:
                    raise error
        else:
    ...
    def set_tags_batch(self, tags, filenames):
        ...
        if isinstance(tags, basestring):
            raise TypeError("The argument 'tags' must be dictionary " "of strings")
        if isinstance(filenames, basestring):
            raise TypeError(
    ...
    def set_keywords_batch(self, mode, keywords, filenames):
        ...
        if isinstance(keywords, basestring):
            raise TypeError("The argument 'keywords' must be " "an iterable of strings")
        if isinstance(filenames, basestring):
            raise TypeError(
                "The argument 'filenames' must be " "an iterable of strings"
            )
        ...
        kw_operation = {KW_REPLACE: "-%s=%s", KW_ADD: "-%s+=%s", KW_REMOVE: "-%s-=%s"}[
            mode
        ]
    ...
    def _check_sanity_of_result(file_paths, result):
        ...
        if len(result) != len(file_paths):
            raise IOError(
                "exiftool did return %d results, but expected was %d"
                % (len(result), len(file_paths))
            )
        for i in range(0, len(file_paths)):
            ...
            if returned_source_file != requested_file:
                raise IOError(
                    "exiftool returned data for file %s, but expected was %s"
