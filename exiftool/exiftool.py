...
class ExifTool(object):
    ...
    def __init__(
        ...
        self._process = (
            None  # this is set to the process to interact with when _running=True
        )
        self._ver = None  # this is set to be the exiftool -v -ver when running

        self._return_tuple: bool = (
            return_tuple  # are we returning a tuple in the execute?
        )
        ...
        self._config_file: Optional[
            str
        ] = None  # config file that can only be set when exiftool is not running
        ...
        self.executable = (
            executable if executable is not None else constants.DEFAULT_EXECUTABLE
        )
    ...
    def executable(self, new_executable) -> None:
        ...
        if Path(new_executable).exists():
            ...
            if abs_path is None:
                raise FileNotFoundError(
                    f'"{new_executable}" is not found, on path or as absolute path'
                )
        ...
        if self._logger:
            self._logger.info(f"Property 'executable': set to \"{abs_path}\"")
    ...
    def block_size(self, new_block_size: int) -> None:
        ...
        if self._logger:
            self._logger.info(f"Property 'block_size': set to \"{new_block_size}\"")
    ...
    def common_args(self, new_args: Optional[List[str]]) -> None:
        ...
        if self._logger:
            self._logger.info(f"Property 'common_args': set to \"{self._common_args}\"")
    ...
    def config_file(self, new_config_file: Optional[str]) -> None:
        ...
        if self.running:
            raise RuntimeError(
                "Cannot set a new config_file while exiftool is running!"
            )
        ...
        if self._logger:
            self._logger.info(f"Property 'config_file': set to \"{self._config_file}\"")
            ...
            if self._process.poll() is not None:
                ...
                if self._logger:
                    self._logger.warning(
                        f"Property 'running': ExifTool process was previously running but died"
                    )
    ...
    logger = property(
        fset=_set_logger, doc="'logger' property to set to the class logging.Logger"
    )
    ...
    def run(self) -> None:
        ...
        if self._common_args:
            proc_args.append(
                "-common_args"
            )  # add this param only if there are common_args
        ...
        if self._logger:
            self._logger.info(
                f"Method 'run': Exiftool version '{self._ver}' (pid {self._process.pid}) launched with args '{proc_args}'"
            )
    ...
    def terminate(self, timeout: int = 30, _del: bool = False) -> None:
        ...
        if _del and constants.PLATFORM_WINDOWS:
            ...
            )  # have to cleanup the process or else .poll() will return None
            # print("after comm")
        ...
        if self._logger:
            self._logger.info(f"Method 'terminate': Exiftool terminated successfully.")
    ...
    def execute(self, *params):
        ...
        signal_num = random.randint(
            100000, 999999
        )  # arbitrary create a 6 digit number (keep it down to save memory maybe)

        # constant special sequences when running -stay_open mode
        seq_execute = f"-execute{signal_num}\n".encode(
            ENCODING_UTF8
        )  # the default string is b"-execute\n"
        seq_ready = f"{{ready{signal_num}}}".encode(
            ENCODING_UTF8
        )  # the default string is b"{ready}"
        ...
        seq_err_post = f"post{signal_num}".encode(
            ENCODING_UTF8
        )  # default there isn't any string
        ...
        if self._logger:
            self._logger.info(
                "Method 'execute': Command sent = {}".format(cmd_text.split(b"\n")[:-1])
            )
        ...
        if self._logger:
            self._logger.debug(
                "Method 'execute': Reply stdout = {}".format(self._last_stdout)
            )
            self._logger.debug(
                "Method 'execute': Reply stderr = {}".format(self._last_stderr)
            )
